...
import type { Goal, PerformanceReview } from "@/lib/store/types";
import { addPerformanceReview, deleteGoal, upsertGoal } from "@/lib/store/hr-actions";
...
export function PerformanceClient({
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-kastros-sand bg-white p-5 shadow-sm">
        <h2 className="font-display text-lg font-semibold text-kastros-forest">Performance grading (manager-led)</h2>
        {session.role === "manager" || session.role === "hr_admin" ? (
          <form className="mt-4 grid gap-3 sm:grid-cols-2" action={(fd) => handle(addPerformanceReview(fd))}>
            <label className="text-sm">
              ...
              {session.role === "manager" ? (
                <p className="mt-1 text-xs text-kastros-sage">Your team: {teamEmails.join(", ") || "No direct reports"}</p>
              ) : null}
            </label>
            <label className="text-sm">
          ...
          <table className="w-full min-w-[720px] text-left text-sm">
            <thead>
              <tr className="border-b border-kastros-sand text-xs uppercase tracking-wide text-kastros-sage">
                ...
                <th className="pb-3 pr-3">Manager</th>
